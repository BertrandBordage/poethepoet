...
class TaskSpecFactory:
    ...
    def create(
        ...
        return PoeTask.lookup_task_spec_cls(task_type)(
            ...
            factory=self,
            parent=parent,
            source=parent.source if parent else source,
        )
    ...
    def load(self, task_name: str):
        ...
        #   - needs to get into the taskspec somehow

        task_def, config_partition = self.config.lookup_task(task_name)
        assert config_partition  # TODO: handle failure here
...
class PoeTask(metaclass=MetaPoeTask):
    ...
    class TaskSpec:
        ...
        def get_task_env(
            ...
            config_dir = self.source.cwd

            result = EnvVarsManager(
                parent_env._config,  # TODO: make EnvVarsManager clone itself
                parent_env._ui,
            ...
            # POE_CONF_DIR must be set before resolving task_envfile and task_env which
            # might reference it
            result.set("POE_CONF_DIR", str(config_dir))
            result.apply_env_config(task_envfile, task_env, config_dir)
